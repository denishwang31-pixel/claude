      ...
      {expanded && (
        <div className="px-5 pb-5 space-y-3 border-t border-cream-100">
          ...
          <SectionBlock
            ...
            content={
              <>
                <p className="font-medium text-cream-800">방법 A: 전체 내역 탭에서 개별 변경</p>
                <ul className="list-disc list-inside space-y-1 mt-1">
                  <li>「전체 내역」 탭 → 각 거래의 카테고리 드롭다운 클릭 → 카테고리 선택</li>
                  <li><strong>해당 거래 1건에만</strong> 적용됩니다.</li>
                  <li>이후 같은 내역명의 새 거래는 변경되지 않습니다.</li>
                </ul>
                <p className="font-medium text-cream-800 mt-3">방법 B: 카테고리 변경 시 팝업 활용</p>
                <ul className="list-disc list-inside space-y-1 mt-1">
                  <li>카테고리를 바꿀 때 <strong>「같은 내역명 전체 일괄 변경」</strong> 옵션을 선택하면, 동일 내역명의 모든 과거 거래에도 적용됩니다.</li>
                </ul>
                <p className="font-medium text-cream-800 mt-3">방법 C: 매핑 규칙 탭에서 규칙 추가</p>
                <ul className="list-disc list-inside space-y-1 mt-1">
                  <li>향후 업로드될 거래에도 자동으로 카테고리가 적용됩니다.</li>
                  <li>기존 거래 중 해당 내역명과 일치하는 건도 소급 적용됩니다.</li>
                </ul>
                <div className="mt-3 p-3 bg-amber-50 rounded-lg text-xs">
                  <strong>변경 효과 범위:</strong> 카테고리가 바뀌면 대시보드 KPI, 월별 요약, 카테고리별 차트, 전체 내역 필터링 등 <strong>모든 화면의 집계에 즉시 반영</strong>됩니다.
                </div>
              </>
            }
          />
          ...
          <SectionBlock
            ...
            content={
              <>
                <p>「매핑 규칙」 탭에서 <strong>내역명 → 카테고리</strong> 매핑 규칙을 관리합니다.</p>
                <ul className="list-disc list-inside space-y-2 mt-2">
                  <li><strong>규칙 추가:</strong> 내역명 키워드와 적용할 카테고리를 지정합니다. 내역명에 해당 키워드가 포함되면 자동으로 카테고리가 지정됩니다.</li>
                  <li><strong>소급 적용:</strong> 규칙을 추가하면 이미 업로드된 거래 중 매칭되는 건도 즉시 카테고리가 변경됩니다.</li>
                  <li><strong>미래 적용:</strong> 이후 CSV를 업로드할 때도 규칙이 자동 적용됩니다.</li>
                  <li><strong>규칙 삭제:</strong> 규칙을 삭제하면 해당 규칙으로 분류됐던 거래는 다음 우선순위(뱅크샐러드 기본 분류)로 되돌아갑니다.</li>
                </ul>
                <div className="mt-3 p-3 bg-cream-50 rounded-lg text-xs">
                  <strong>활용 예시:</strong> 「스타벅스」 → 「식비」, 「GS25」 → 「편의점」, 「황동현」 → 「저축」 등으로 설정해두면 반복 거래를 수동으로 바꿀 필요가 없습니다.
                </div>
              </>
            }
          />
          ...
          <SectionBlock
            ...
            content={
              <ul className="list-disc list-inside space-y-2">
                <li><strong>수입은 단일 카테고리:</strong> 카테고리별 화면에서 수입은 항상 「수입」 하나로 집계됩니다. 어떤 카테고리로 바꿔도 금액이 양수이면 수입으로 분류됩니다.</li>
