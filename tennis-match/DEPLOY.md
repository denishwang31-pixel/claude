...
`eas build` 는 프로젝트를 EAS 서버로 보내 거기서 빌드한다. `app.config.js`
도 **EAS 서버에서** 평가되므로, GitHub Actions 의 환경변수는 닿지 않는다.
그래서 **expo.dev 의 프로젝트 환경변수에도 같은 값을 넣어야 한다.**

👉 https://expo.dev/accounts/denis-hwang/projects/tennis-match/environment-variables
...
앱에서 **[더보기] → 앱 정보 → 소셜 로그인**. 여기가 유일하게 믿을 수 있는
자리다 — 빌드된 앱 안에서 실제로 박힌 값을 보기 때문이다.

빌드 로그의 표는 **GitHub 쪽에 키가 있는지만** 본다. 그 표가 ✅ 여도 EAS
환경변수가 비어 있으면 앱에는 안 들어간다. 실제로 그렇게 한 번 속았다.
