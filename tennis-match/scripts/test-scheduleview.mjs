...
section('현황 요약 — 명단을 안 그려도 상태를 안다');
{
  ...
  eq('참석', s.yes, 1);
  eq('불참', s.no, 1);
  /* ⚠️ '미정'은 고를 수 없게 됐다. 예전에 저장된 값은 **미응답에 포함**해
     센다 — 답이 아니기 때문이다. maybe 는 그 안의 부분집합이라 따로도
     세어 둔다("예전 미정이 몇 명 남았나"를 알아야 다시 물어본다).
     그래서 none 은 m4(미정) + m5(무응답) = 2 이고, answered 는 참석·불참뿐이다.
     ⚠️ 미정을 그냥 빼 버리면 그 사람은 참석·불참·미응답 어디에도 안
        들어가 명단에서 조용히 사라진다. 인원을 세는 화면에서 사람이
        사라지는 것은 대진이 어긋나는 일이다. */
  eq('예전 미정은 따로도 센다', s.maybe, 1);
  eq('미응답 — 예전 미정을 포함한다', s.none, 2);
  eq('응답 수 — 참석·불참만', s.answered, 2);
  eq('게스트', s.guests, 1);
