...
/* ============================================================
   ...
   ============================================================ */
/* 앱 이름 — 화면·초대 문구·약관·웹이 모두 이 값을 쓴다.
   스토어 이름을 바꾸면 여기(+ app.json 의 name)만 고친다. 예전에 "테니스매치"가
...
export const POLL_TYPE = {
  ATTEND: 'attend',   // 참가 여부 (참가/불참)
  CHOICE: 'choice',   // 선택지 투표
};
/**
 * 참가 여부 투표의 선택지.
 *
 * ⚠️ '미정'을 뺐다. 취향이 아니라 운영 문제다 — 선택지에 미정이 있으면
 *    확실하지 않은 사람이 전부 거기로 몰린다. 고르기 제일 편하고 아무
 *    책임도 안 지는 칸이기 때문이다. 그러면 총무는 몇 명이 오는지 모른
 *    채로 코트를 잡아야 하고, 결국 단톡방에서 한 명씩 다시 묻게 된다.
 *    투표를 한 이유가 사라진다.
 *
 * ⚠️ 이미 '미정'으로 찍힌 표가 있다. 그래서 아래 값을 지우지 않았다 —
 *    지우면 그 표는 어느 칸에도 안 들어가서 "12명 참여"인데 칸 합은
 *    9명인 상태가 된다. 숫자가 안 맞는 화면은 아무도 못 믿는다.
 *    예전 표가 있을 때만 읽기 전용 칸으로 같이 보여 준다.
 */
...
/** 예전에 찍힌 '미정' 표를 보여 줄 때만 쓴다. 새로 고를 수는 없다. */
export const POLL_ATTEND_LEGACY = { key: 'maybe', label: '미정 (예전 응답)' };

/** 회비 납부 주기 */
export const FEE_CYCLE = { MONTHLY: 'monthly', YEARLY: 'yearly' };

/** 용품 광고 카테고리 */
/* 용품 카테고리 — 앱 주인이 정한 묶음(라켓 / 악세사리 / 보강재료 / 보조)을
   매장에서 흔히 쓰는 말로 맞췄다.
     「보강재료」(납·스트링 세이버 등) → 「라켓 튜닝」: 납작업·댐퍼를 흔히 "튜닝"이라 부른다
     「보조」(무릎·손목 보호대)        → 「보호대」: 찾는 사람이 먼저 떠올리는 말
     「악세사리」                      → 표준어 「액세서리」
   ⚠️ 예전 값 '스트링·소모품' 은 gearView.js 가 '스트링'으로 읽는다(지우지 말 것). */
export const GEAR_CATEGORIES = [
  '라켓', '스트링', '그립', '라켓 튜닝', '보호대', '신발', '의류', '가방', '공', '액세서리', '기타',
];
/** 등록할 때 카테고리 아래 보여 줄 예 — 어디에 넣을지 헷갈리지 않게 */
export const GEAR_CATEGORY_HINT = {
  라켓: '완성 라켓',
  스트링: '폴리·멀티·나일론·거트',
  그립: '오버그립·리플레이스먼트 그립',
  '라켓 튜닝': '납 테이프·진동 댐퍼·스트링 세이버',
  보호대: '무릎·손목·팔꿈치 보호대·테이핑',
  신발: '테니스화·깔창',
  의류: '상의·하의·스커트·양말',
  가방: '라켓 가방·백팩',
  공: '테니스공·볼 캔',
  액세서리: '모자·손목밴드·볼 홀더·타월',
  기타: '',
};
